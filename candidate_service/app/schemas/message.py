...
from .user import PyObjectIdStr # This should work as user.py is in the same directory
...
class MarkUnreadRequest(BaseModel):
    message_ids: List[PyObjectIdStr] = Field(
        ..., min_length=1, description="List of message IDs to mark as unread"
    )

    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                "message_ids": ["681cae4f5634b1e5b7c8d01a"]
            }
        }
    )
