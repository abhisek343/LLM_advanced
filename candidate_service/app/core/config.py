...
class Settings(BaseSettings):
    APP_NAME: str = "LLM Interviewer API - Candidate Service" # Modified for service
    API_V1_STR: str = os.getenv("CANDIDATE_API_V1_STR", "/api/v1") # Read from env var, default to /api/v1
    LOG_LEVEL: str = "INFO"
    ...
    RESUME_SUBDIR: str = "resumes" # Candidate service will use this
...
def get_settings() -> Settings:
    logger.info("Loading application settings for Candidate Service...")
    try:
        ...
        logger.info(f"--- Candidate Service DEBUG: Settings() instance created. Gemini Key: {log_key_display_settings} ---")
        return settings_instance
    except Exception as e:
        logger.critical(f"FATAL: Failed to load settings for Candidate Service: {e}", exc_info=True)
        raise SystemExit(f"Could not load settings for Candidate Service: {e}")
