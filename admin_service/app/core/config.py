...
class Settings(BaseSettings):
    ...
    # Gemini settings might not be directly used by Admin service, but kept for consistency
...
def get_settings() -> Settings:
    logger.info("Loading application settings for Admin Service...")
    try:
        ...
        logger.info(f"--- Admin Service DEBUG: Settings() instance created. Gemini Key: {log_key_display_settings} ---")
        return settings_instance
    except Exception as e:
        logger.critical(f"FATAL: Failed to load settings for Admin Service: {e}", exc_info=True)
        raise SystemExit(f"Could not load settings for Admin Service: {e}")
