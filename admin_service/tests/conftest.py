...
_SERVICE_TEST_MONGODB_DB_NAME = f"{_SERVICE_ORIGINAL_MONGODB_DB_NAME_IN_SETTINGS}_admin_test"

@pytest_asyncio.fixture(scope="session", autouse=True)
async def manage_service_test_db_settings_admin():
...
async def test_db_client_admin(manage_service_test_db_settings_admin) -> AsyncGenerator[AsyncIOMotorClient, None]:
    if mongodb.client:
        await mongodb.close()
    await mongodb.connect()
    if not mongodb.client:
        raise RuntimeError(f"Failed to establish test_db_client for admin_service ({settings.MONGODB_DB}).")
...
async def client_admin(test_db_client_admin: AsyncIOMotorClient) -> AsyncGenerator[AsyncClient, None]:
    # Corrected line: Use ASGITransport for in-process testing with AsyncClient
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://testserver-admin") as ac:
        yield ac
