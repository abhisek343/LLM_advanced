...
class MongoDB:
    ...
    def __init__(self):
        ...
        logger.info("MongoDB manager initialized for HR Service.")

    async def connect(self):
        if self.client is not None:
            logger.warning("HR Service: Connection attempt ignored, MongoDB client already initialized.")
            return

        if not self.mongodb_url or not self.mongodb_db_name:
             logger.error("HR Service: MONGODB_URL or MONGODB_DB not configured in settings.")
             raise ValueError("MongoDB connection details missing in settings.")

        try:
            logger.info(f"HR Service: Attempting to connect to MongoDB at {self.mongodb_url}...")
            ...
            logger.info(f"HR Service: MongoDB connection successful. Database '{self.mongodb_db_name}' is ready.")

        except Exception as e:
            logger.error(f"HR Service: Failed to connect to MongoDB: {e}", exc_info=True)
            self.client = None
            self.db = None
            raise
    ...
    async def close(self):
        if self.client:
            ...
            logger.info("HR Service: No active MongoDB connection to close.")

    def get_db(self) -> AsyncIOMotorDatabase:
        if self.db is None:
            logger.critical("HR Service: get_db called but database is not connected/initialized.")
