...
class Settings(BaseSettings):
    APP_NAME: str = "LLM Interviewer API - Interview Service" # Modified for service
...
def get_settings() -> Settings:
    logger.info("Loading application settings for Interview Service...")
    try:
        ...
        logger.info(f"--- Interview Service DEBUG: Settings() instance created. Gemini Key: {log_key_display_settings} ---")
        return settings_instance
    except Exception as e:
        logger.critical(f"FATAL: Failed to load settings for Interview Service: {e}", exc_info=True)
        raise SystemExit(f"Could not load settings for Interview Service: {e}")
