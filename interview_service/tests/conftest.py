...
_SERVICE_TEST_MONGODB_DB_NAME = f"{_SERVICE_ORIGINAL_MONGODB_DB_NAME_IN_SETTINGS}_interview_test"

@pytest_asyncio.fixture(scope="session", autouse=True)
async def manage_service_test_db_settings_interview():
...
async def test_db_client_interview(manage_service_test_db_settings_interview) -> AsyncGenerator[AsyncIOMotorClient, None]:
    if mongodb.client:
        await mongodb.close()
    await mongodb.connect()
    if not mongodb.client:
        raise RuntimeError(f"Failed to establish test_db_client for interview_service ({settings.MONGODB_DB}).")
...
async def client_interview(test_db_client_interview: AsyncIOMotorClient) -> AsyncGenerator[AsyncClient, None]:
    # Corrected line: Use ASGITransport for in-process testing with AsyncClient
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://testserver-interview") as ac:
        yield ac
